...
Write-Host "Running Stage 9: Train 4DGS" -ForegroundColor Cyan
Write-Host "Script: train_queue.py"
Write-Host "Environment: 4dgs"

Set-Location $GaussiansAbsPath
conda run -n 4dgs --no-capture-output python "train_queue.py" $args
Set-Location $RepoRoot
