...
def plot_force_over_time(force_series: List[float], experiment_name: str, output_dir: str, total_experiment_time: float):
    ...
    plt.plot(time_points, force_series, marker='o', linestyle='-', color='red', linewidth=3, markersize=8)
    plt.xlabel('Time (s)', fontsize=16)
    plt.ylabel('Axial Force (N)', fontsize=16)
    plt.title(f'Axial Force Over Time - {experiment_name}', fontsize=18, fontweight='bold')
...
def run_force_analysis(config_manager):
    ...
    if use_cluster_export:
        ...
        if export_path:
            ...
            # Still need labels for compatibility, but they won't be used for computation
            # Create dummy labels matching the cluster count
            labels = np.zeros(len(xyz), dtype=int)
            cluster_points = [np.array([]) for _ in range(len(cluster_means))]
        else:
    ...
    if saved_edges is not None and len(saved_edges) > 0:
        print(f"Using saved edges: {len(saved_edges)} edges")
        vertices = cluster_means
    ...
    print(f"Using mesh with {len(valid_edges)} valid edges")
    ...
    if use_cluster_export:
        ...
        for j in range(1, num_frames):
            ...
            axial_force = calculate_force_through_plane(new_cluster_means, valid_edges, stress, final_camera_position, area_method)
