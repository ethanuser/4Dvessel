...
YOUNG_MODULUS_SILICON = 1.15e6
OUTLIER_EDGE_THRESHOLD = 0.25
MAX_STRESS_PA = 0.15e6  # 0.15 MPa in Pa (default max stress for colorbars)
...
def print_progress_bar(current, total, bar_length=40):
    ...
    sys.stdout.write(f'\rProgress: [{hashes}{spaces}] {current}/{total} ({percent*100:.1f}%)')
    sys.stdout.flush()
...
def get_colormap():
    """
    Returns the configured colormap.
    """
