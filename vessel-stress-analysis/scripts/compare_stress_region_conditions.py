...
CSV_PATH = os.path.join(project_root, 'data', 'processed', 'max_region_analysis', 'max_stresses_regions_pivot_scaled.csv')
...
def load_stress_data(csv_path):
    """
    Load and structure the stress data from CSV.
...
def create_visualizations(data, all_results, save_dir=None):
    ...
    for loc_idx, location in enumerate(locations):
        ...
        sns.boxplot(data=location_df, x='Condition', y='Stress (MPa)', 
                   ax=ax, palette='Set2', width=0.6)
        sns.stripplot(data=location_df, x='Condition', y='Stress (MPa)', 
        ...
        sns.violinplot(data=location_df, x='Condition', y='Stress (MPa)',
                      ax=ax, palette='Set2', inner='box')
        ax.set_title(f'{location}\nDistribution Shape', fontweight='bold', fontsize=11, pad=10)
        ax.set_ylabel('Stress (MPa)', fontsize=10)
        ax.set_xlabel('Condition', fontsize=10)
    
    fig1.suptitle('Maximum Stress Analysis by Location: Three Separate ANOVAs\n(Cervical vs Terminal vs Cavernous)', 
                 fontsize=16, fontweight='bold', y=0.995)
    
    fig1.tight_layout(rect=[0, 0, 1, 0.97])
    
    save_path1 = os.path.join(save_dir, 'stress_comparison_by_location.png')
    fig1.savefig(save_path1, dpi=300, bbox_inches='tight')
    fig1.savefig(save_path1.replace('.png', '.svg'), format='svg', bbox_inches='tight')
    print(f"  ✓ Figure 1: stress_comparison_by_location.png/.svg")
    ...
    ax1.set_title('Mean Maximum Stress ± SEM by Location and Condition', 
    ...
    ax3.set_title('Heatmap of Mean Maximum Stress', fontsize=12, fontweight='bold', pad=12)
    ...
    cbar = plt.colorbar(im, ax=ax3, label='Stress (MPa)')
...
def export_summary_table(all_results, data, save_path=None):
    ...
    Args:
        ...
        save_path: Path to save the CSV file (defaults to statistical_plots/stress_statistics_summary.csv)
    """
    # Default to data/processed/max_region_analysis/statistical_plots
    if save_path is None:
        ...
        save_dir = os.path.join(project_root, 'data', 'processed', 'max_region_analysis', 'statistical_plots')
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, 'stress_statistics_summary.csv')
    ...
    print(f"  ✓ CSV: stress_statistics_summary.csv")


def main():
    """Main function to run the stress comparison analysis."""
    ...
    print("="*80)
    print("MAXIMUM STRESS ANALYSIS: THREE SEPARATE ONE-WAY ANOVAs")
    print("Comparing Cervical vs Terminal vs Cavernous at Each Location")
    print("="*80)
    
    # Load data
    print(f"\nLoading data from: {csv_path}")
    data = load_stress_data(csv_path)
